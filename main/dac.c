...
void i2s_init() {
	...
	i2s_config_t i2s_config = { .mode = I2S_MODE_MASTER | I2S_MODE_RX
			| I2S_MODE_TX | I2S_MODE_DAC_BUILT_IN | I2S_MODE_ADC_BUILT_IN,
			.sample_rate = EXAMPLE_I2S_SAMPLE_RATE, .bits_per_sample =
			EXAMPLE_I2S_SAMPLE_BITS, .communication_format =
					I2S_COMM_FORMAT_I2S_MSB, .channel_format =
			EXAMPLE_I2S_FORMAT, .intr_alloc_flags = 0, .dma_buf_count = 2,
			.dma_buf_len = 1024 };
...
void play_wav(void *pvParameter) {
	...
	uint8_t* i2s_write_buff = (uint8_t*) calloc(EXAMPLE_I2S_READ_LEN,
			sizeof(char));
	while (1) {
		...
		while (offset < tot_size) {
			int play_len =
					((tot_size - offset) > (4 * 1024)) ?
							(4 * 1024) : (tot_size - offset);
			int i2s_wr_len = i2s_dac_data_scale(i2s_write_buff,
					(uint8_t*) (player_defeated + offset), play_len);
